...
| 0099 | [0099_static_purity_completion.md](0099_static_purity_completion.md) | In Progress | partial | Parts 1 and 2 are closed; Part 3 monomorphization remains active purity/runtime work | keep as purity umbrella, not as the active static-typing roadmap |
...
| 0015 | [0015_package_module_workflow_mvp.md](0015_package_module_workflow_mvp.md) | Superseded | partial | module roots (`--root`) exist; package workflow not present. Document is normalization-damaged (duplicated headings, elided body) and not implementable as written | superseded by [0177](implemented/0177_package_manager.md) |
...
| 0161 | [0161_effect_system_decomposition_and_capabilities.md](implemented/0161_effect_system_decomposition_and_capabilities.md) | Implemented | implemented | refreshed 2026-04-23: compiler-seeded decomposition, sealing, strict `Flow` audit slice, shared compiler/Aether Phase 3 registry cleanup, intrinsic-backed `Flow.Primops`, prelude scheme authority, and `examples/sealing/` fixtures | Remaining follow-up: remove bootstrap Rust HM fallback once all direct compiler entry paths preload `Flow.Primops`; no `extern fn`; `Random`/`NonDet`/`Exn` reserved until real ops/primops land; supersedes 0075, 0108, 0131 |
...
| 0166 | [0166_pattern_exhaustiveness_and_redundancy_checking.md](implemented/0166_pattern_exhaustiveness_and_redundancy_checking.md) | Implemented | have | matrix-based coverage checker (Maranget-style) in `src/ast/type_infer/pattern_coverage.rs` + adapter; ~755 lines of ad-hoc checks decommissioned; E015 unified, W202/E083 removed | closed 2026-04-22 |
| 0167 | [0167_static_typing_contract_hardening.md](implemented/0167_static_typing_contract_hardening.md) | Implemented | have | `BoundaryKind` enum consumed by `StrictTypeValidator` and Core-adjacent contract pass; unified `ranking::is_suppressed_by`; `ExprIdGen::resuming_past_program` for synthetic AST; Core contract pass default-on at Error severity | closed 2026-04-22 |
| 0168 | [0168_hkt_polymorphic_dispatch_completion.md](implemented/0168_hkt_polymorphic_dispatch_completion.md) | Implemented | have | HKT constructor variables bind via new `HktApp` arm in `match_constraint_type_var`; AST-fallback dict insertion for constrained user-function calls; regression test `hkt_constrained_polymorphic_call_elaborates_dictionary` | closed 2026-04-22 |
| 0169 | [0169_parameterized_handlers.md](implemented/0169_parameterized_handlers.md) | Implemented | have | parameterized effect handlers (`handle Eff(init) { arm(resume, …, state) → … }`) landed across parser, HM, Core, Aether, CFG/VM, and native lowering; full `cargo test` and `cargo test --all --all-features` passed | closed 2026-04-24; remaining polish documented in proposal follow-ups |
| 0165 | [0165_io_primop_migration_to_effect_handlers.md](implemented/0165_io_primop_migration_to_effect_handlers.md) | Implemented | have | effectful prelude primops route through `Perform`; entrypoints/tests get default handlers; `Flow.Primops.__primop_*` is the intrinsic-backed implementation layer; `examples/effects/` documents behavior | closed 2026-04-24; polish tracked by 0171 |
| 0170 | [0170_polymorphic_effect_operations.md](implemented/0170_polymorphic_effect_operations.md) | Implemented | have | `effect_op_signatures` stores `Scheme`; effect ops generalize at collection time and instantiate fresh at `perform` / handler sites; regression coverage in HM and compiler tests | keep as baseline for polymorphic effect operations |
| 0171 | [0171_effect_system_polish_and_hardening.md](0171_effect_system_polish_and_hardening.md) | Draft | gap | follow-up proposal split from 0165 review | improve source-aware diagnostics, handler ergonomics, `Flow.Primops` documentation, shared effect availability checks, entry-default negative coverage, and default-handler policy |
| 0172 | [0172_test_suite_reorganization_by_compiler_phase.md](implemented/0172_test_suite_reorganization_by_compiler_phase.md) | Implemented | have | flat `tests/` reorganized into pipeline-aligned subdirectories; snapshot shims removed, duplicate clusters merged, helpers extracted to `tests/support/` | closed 2026-04-21; originally filed as 0169 and renumbered to avoid collision |
...
| 0152 | [0152_named_fields_for_data_types.md](implemented/0152_named_fields_for_data_types.md) | Implemented | have | named fields + dot-access + functional update syntax; supersedes 0048 | closed 2026-04-18; known gap: named-field syntax fails in the REPL (E082) — see "Known gaps" |
...
| 0177 | [0177_package_manager.md](implemented/0177_package_manager.md) | Implemented | have | Phases 0–3 shipped 2026-08-26; hosted registry transport intentionally deferred pending KI-035; supersedes 0015 | Package manager: `flux.toml`, `flux.lock`, namespaced packages, path and Git dependencies, local registry index resolution, `add/remove/update/tree`, offline/locked/frozen modes, content-addressed store, workspaces, metadata, build plans, and `publish --dry-run`. Remaining questions are tracked as KI-042–KI-049; hosted registry transport remains intentionally deferred until Flux is released. |
...
| 0179 | [0179_typeclass_soundness_dictionary_passing_and_associated_types.md](implemented/0179_typeclass_soundness_dictionary_passing_and_associated_types.md) | Implemented | have | Stages 0-3 and 5-8 shipped: polymorphic dictionary passing works on both backends, predicates keep their full type arguments end to end, superclass evidence occupies dictionary slots, kinds are checked, associated types reduce or stay stuck, deriving is validated, and `Eq`/`Ord`/`Num`/`Show`/`Semigroup`/`Monoid`/`Functor`/`Applicative`/`Monad` live in `lib/Flow/*.flx`. All nine stages shipped: Stage 4's result-directed selection reads the type a call's result is required to have, from the enclosing return type or `let` binder, so two dictionaries for one class are told apart; a method naming its class parameter nowhere remains `E485`. KI-015 and KI-057 to KI-061 closed; KI-069 and KI-070 no longer reproduce either (verified 2026-09-11), leaving no open descendant | complete typeclass direction: one evidence pipeline, sound constraint solving, superclass entailment with dictionary slots, real kind checking, result-directed dispatch (closed KI-015), associated types, safe deriving, and the `Eq→Ord` / `Semigroup→Monoid` / `Functor→Applicative→Monad` hierarchy |
| 0180 | [0180_flume_distribution_and_repository_split.md](0180_flume_distribution_and_repository_split.md) | Draft | partial | `flux --native --emit-binary` already produces a working standalone `flume` (verified 2026-08-27: `init`/`add`/`tree`/`entry`/`profile` run outside the checkout); no install script exists in `scripts/release/`; `flux → flume` dependency still inverted vs `cargo → rustc` | how Flume is distributed: relocatable `<prefix>/bin` + `<prefix>/lib` install layout (phase 1, needed regardless), a separate `flume` binary that spawns `flux` (phase 2), and a framed replacement for the `ok<TAB>` reply protocol (phase 3). Recommends **against** extracting `lib/Flume/` to its own repository: 12 of the last 15 Flume commits also changed `src/` |
| 0181 | [0181_documentation_generation.md](0181_documentation_generation.md) | Draft | partial | `///` and `/** */` lex to `TokenType::DocComment` and index by declaration line (`src/syntax/program.rs:20`); 665 `///` lines across 21 of 30 `lib/Flow` modules; LSP hover/completion/signature-help consume them; `//!` and `/*!` currently swallowed as ordinary comments (verified 2026-08-27: 0 occurrences in `lib/`, `examples/`, `tests/`); no `flux doc` command, no HTML generator | `flux doc` generating a static HTML site: `//!` / `/*!` module doc syntax (phase 1), a structured model in a new `src/doc/` (phase 2), HTML MVP with `pulldown-cmark` behind a `doc` feature (phase 3), dependencies and cross-links (phase 4), search and source pages (phase 5). Enumerates the **filesystem**, not the import graph — 5 of 30 stdlib modules have no importer and an entry-rooted walk drops them silently. Recommends **against** storing docs in `.flxi`: `interface_fingerprint` excludes `source_hash`, so cached docs would go stale with no invalidation path |
| 0182 | [0182_typeclass_syntax_completeness.md](0182_typeclass_syntax_completeness.md) | Draft | partial | Every type class surface form audited and run 2026-09-03, recorded in [type_class_syntax.md](../internals/type_class_syntax.md) with `examples/type_classes/syntax_tour.flx` as its executable half. Supported: multi-parameter classes, one superclass, default method bodies (previously undocumented), higher-kinded parameters, associated types, contextual instances, all four constraint spellings, `deriving`. Rejected: 6 forms, all the same parser decision — `parse_class_statement` and `parse_instance_statement` each build a one-element `vec![constraint]`, no loop, no parenthesised list; superclass args come from bare identifiers only. Audit also produced KI-071 to KI-076 | a constraint *list* before `=>` in class and instance heads (parenthesised when plural, current spelling unchanged when single), structured superclass arguments via the existing `parse_instance_type_args`, superclass evidence extended from one leading dictionary slot to a list, and a decision on whether `where` becomes the single spelling for class contexts — the question 0179 left open. Excludes `class Box<a: Eq>`: meaning unclear, no demand |
| 0183 | [0183_constraint_solver_terminal_states.md](0183_constraint_solver_terminal_states.md) | Draft | partial | Stage 1 implemented on `fix/phase1-promote-e442`: quantification now consumes the predicate, gated on `GeneralizationMode::Definition`, taking terminal stuck from 13,737 to 4,443 corpus-wide and from 68 to 22 on `fn main() { print(1) }`. Stages 2-3 unimplemented. Measurement recorded 2026-09-03 over `examples/type_classes` and `tests/parity` | give every wanted class constraint a terminal outcome, removing `Disposition::Stuck` as a whole-program state. Follows GHC's `simplifyTop` structure: quantification removes a predicate from the wanted set (THIH's `split`, the step Flux was missing), defaulting resolves what it can, and the residue is reported. Closes the escalation proposal [0179](implemented/0179_typeclass_soundness_dictionary_passing_and_associated_types.md) Stage 3 promised but did not implement |
...
| 0075 | [0075_effect_sealing.md](superseded/0075_effect_sealing.md) | Superseded | [0161_effect_system_decomposition_and_capabilities.md](implemented/0161_effect_system_decomposition_and_capabilities.md) | folded into 0161 Phase 2 (2026-04-18) |
| 0108 | [0108_base_function_effect_audit.md](superseded/0108_base_function_effect_audit.md) | Superseded | [0161_effect_system_decomposition_and_capabilities.md](implemented/0161_effect_system_decomposition_and_capabilities.md) | folded into 0161 Phase 1.5 (2026-04-18) |
| 0131 | [0131_primop_effect_levels.md](superseded/0131_primop_effect_levels.md) | Superseded | [0161_effect_system_decomposition_and_capabilities.md](implemented/0161_effect_system_decomposition_and_capabilities.md) | folded into 0161 Phase 3 — derive from effect rows (2026-04-18) |
