...
1. **`FluxHeader.refcount` stays the last field, naturally aligned.** When/if the field becomes `_Atomic(int32_t)` in Phase 5, no layout change. Mirrors Koka ([kklib/include/kklib.h:101-135](https://github.com/koka-lang/koka/blob/master/kklib/include/kklib.h)).
...
- **Koka** — `lib/v1/std/async.kk:535-540` is the direct architectural model: `await(setup)` + `cancelable` + libuv + atomic RC. ([https://github.com/koka-lang/koka](https://github.com/koka-lang/koka))
