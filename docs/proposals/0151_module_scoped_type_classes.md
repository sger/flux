...
Koka does not have Haskell-style type classes. It solves the "a type must support operation X" problem with **named implicit parameters** (`?foo : ...`) and **named handlers** for effectful dictionaries. Observed in [https://github.com/koka-lang/koka/blob/master/lib/std/core/list.kk](https://github.com/koka-lang/koka/blob/master/lib/std/core/list.kk):
...
- Named handlers (`named effect reader`, `named handler ... fun ask() msg`, see [https://github.com/koka-lang/koka/blob/master/samples/handlers/named/ask.kk](https://github.com/koka-lang/koka/blob/master/samples/handlers/named/ask.kk)) give Koka multiple simultaneous "instances" via runtime evidence handles, which is more flexible than a single-canonical-instance model.
