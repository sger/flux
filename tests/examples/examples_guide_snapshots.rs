...
#[test]
fn examples_guide_compile_snapshots() {
    let (_lock, _guard) = diagnostics_env::with_no_color(Some("1"));

    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR"));
    let cases = examples_snapshot::run_fixture_dir_snapshots(workspace_root, "examples/guide")
        .unwrap_or_else(|e| panic!("{e}"));

    for case in cases {
        insta::with_settings!({
            snapshot_path => "../snapshots/examples_guide",
