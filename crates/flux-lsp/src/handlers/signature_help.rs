...
//! a call's arguments.
//!
//! The signature label is built from the callee's inferred function type. When
//! the callee is declared in this buffer we enrich it with the real parameter
//! names and its `///` doc comment; otherwise it falls back to types only. Each
...
pub fn signature_help(snapshot: &Snapshot, position: Position) -> Option<SignatureHelp> {
    ...
    // Parameter names + decl span from the callee's declaration in this buffer
    // (matched by name and arity). `None` for stdlib / imported / computed
    // callees we can't resolve here — those show types only.
    let decl = name
        .as_deref()
        .and_then(|n| function_decl(&snapshot.program, &snapshot.interner, n, param_types.len()));
    ...
    for (idx, ty) in param_types.iter().enumerate() {
        ...
        if let Some((names, _)) = &decl {
            label.push_str(&names[idx]);
            label.push_str(": ");
        }
    ...
    let documentation = decl.as_ref().and_then(|(_, span)| {
        doc_comment_above(
