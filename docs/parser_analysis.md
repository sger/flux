...
## 6) Ranked Candidate Heuristics (Plan Only, No Code)

### 1. Expression-context awareness flag (highest impact)
Insertion points:
- `parse_expression` (`src/syntax/parser/expression.rs:66`)
- `parse_prefix` (`src/syntax/parser/expression.rs:89`)
- Call sites in `parse_if_expression`, `parse_match_expression`, `parse_lambda`, `parse_expression_statement`.

Behavior:
- Carry lightweight context (`ExprPos`, `StmtPos`, maybe `ArmBodyPos`).
- When `let` appears in `ExprPos`, emit targeted diagnostic: “`let` is statement-only here; wrap in `{ ... }` or rewrite.”
- Avoid generic E031 where we can classify intent.

### 2. Let-in-expression targeted diagnostic shim
Insertion points:
- `parse_prefix` default branch (`src/syntax/parser/expression.rs:124`)
- `no_prefix_parse_error` path (`src/syntax/parser/helpers.rs:610`)

Behavior:
- Special-case `TokenType::Let` before generic no-prefix error.
- Keep parse result behavior unchanged for phase 2 (still error), but improve message specificity and hint.
- Reduce ambiguity with E031 spam.

### 3. Delimiter-balanced local recovery scopes
Insertion points:
- `parse_grouped_expression` (`src/syntax/parser/expression.rs:365`)
- `parse_parenthesized` helper (`src/syntax/parser/expression.rs:21`)
- `parse_match_expression` arm loop (`src/syntax/parser/expression.rs:612`)

Behavior:
- On missing expected close delimiter, recover to nearest safe boundary at same nesting depth before returning.
- Prefer local recovery over full statement sync where possible.
- Minimize downstream “expected expression/unexpected token” cascades.

### 4. Shared “expected delimiter after line break” heuristic
Insertion points:
- `peek_error` (`src/syntax/parser/helpers.rs:654`)
- list parser branch `likely_missing_closing_delimiter` (`src/syntax/parser/helpers.rs:515`)

Behavior:
- If expected `)`/`]` and next token begins a new statement line, classify as “likely missing closing delimiter.”
- Emit one primary diagnostic and suppress secondary generic delimiter diagnostics.
- Reuse across grouped expressions, call args, list literals.

### 5. Match-arm body context guard
Insertion points:
- `parse_match_expression` body parse (`src/syntax/parser/expression.rs:627`)

Behavior:
- If arm body starts with statement-only tokens (`let`, `return`), emit targeted message (“match arm body expects expression”).
- Suggest `{ ... }` form if language intends statement blocks in arms in future.
- Reduce E031 ambiguity in `match` contexts.

### 6. Lambda body context guard
Insertion points:
- `parse_lambda` expression-body branch (`src/syntax/parser/expression.rs:913`)

Behavior:
- When non-block lambda body starts with statement-only token, emit targeted lambda-body diagnostic.
- Keep current grammar unchanged (no let-expression support), but produce direct guidance.

### 7. Statement-start sync set centralization
Insertion points:
- `token_starts_statement` (`src/syntax/parser/helpers.rs:149`)
- `synchronize` and list recovery stop conditions (`src/syntax/parser/helpers.rs:161`, `src/syntax/parser/helpers.rs:550`).

Behavior:
- Reuse one authoritative “statement start” predicate for all recoveries.
- Prevent drift between different recovery branches.
- Improve consistency of where parser resumes.

### 8. Bounded speculative parse at ambiguous `(` sites
Insertion points:
- `parse_grouped_expression` (`src/syntax/parser/expression.rs:365`)
- possibly `parse_pattern` tuple branch (`src/syntax/parser/expression.rs:771`)

Behavior:
- Speculate a short distance to classify likely grouping vs malformed tuple vs missing `)`.
- On failure, roll back token cursor only within small bounded window.
- Emit clearer primary diagnostics for `(`-related ambiguity.

### 9. Error coalescing key for duplicate cascades
Insertion points:
- central diagnostic push sites in parser (`peek_error`, list parser, member access).

Behavior:
- Coalesce near-duplicate diagnostics by `(code, line, expected-token-class)`.
- Keep first/root error, suppress repetitive follow-ups on same line span.
- Preserve all current parse branches while reducing noise.

### 10. Optional newline-as-soft-terminator guard (only where safe)
Insertion points:
- `parse_expression_list_core` separator checks (`src/syntax/parser/helpers.rs:383`)
- `parse_member_access` newline check (`src/syntax/parser/expression.rs:276`)

Behavior:
- Treat line break as stronger signal to stop list/member continuation in select contexts.
- Must remain conservative to avoid changing valid multiline expression behavior.
- Primarily a recovery heuristic, not grammar change.
