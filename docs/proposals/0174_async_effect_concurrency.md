...
| **Phase 1b** — Fiber layer + structured concurrency | 🟡 partial | Slices 1b-i through 1b-v landed (effect seams, FiberScheduler, 5 fiber CorePrimOps, Flow.Async source surface). 1b-vi-a/b₁/b₂.1/b₂.2 and a follow-up `Async.timeout` slice landed on the **VM path**: `Async.sleep` routes through the mio reactor; every `Async.run_async` boundary owns a real `FiberScheduler`; `FiberSleep`, `FiberBoth`, `FiberRace`, `FiberTimeout` capture continuations and park/resume through a dispatch loop, giving genuine wall-clock overlap (`both(sleep(500), sleep(500))` ≈ 500ms; `timeout(50, sleep(1000))` returns `None` in ~50ms). What remains: **native (LLVM) fiber suspend/resume** (1b-vi-d), **per-worker queues + multi-OS-thread workers** (1b-vi-c), **cancellation propagation** (race losers / scope cancellation / bracket+timeout), and **broader TCP parity coverage** (1b-vi-e). |
...
| 1b-vi-d — Native (LLVM) fiber suspend/resume | ⏳ | `flux_fiber_*` C runtime functions (`flux_fiber_run_async`, `flux_fiber_sleep`, `flux_fiber_fork`, `flux_fiber_both`, `flux_fiber_race`, `flux_fiber_timeout`). LLVM emit-side stubs already exist (linker fails loudly until the C side ships); the actual park/resume cycle on the native backend is the substantive work. |
| 1b-vi-e — Real TCP parity | ⏳ | TCP echo at scale, parallel TCP fetch, cancellation under load. Today the TCP surface is wired but only smoke-tested. |
| **Phase 2** — HTTP/1.1 + JSON + Streams | ⏳ | |
| **Phase 3** — TLS + database client | ⏳ | |
| **Phase 4** — `io_uring` backend (optional) | ⏳ | |

Test count after Phase 1b 1b-vi-a + b₁ + b₂.1 + b₂.2 + timeout slice: **2491 passed / 0 failed** under `cargo test --all --all-features` (2485 baseline + 6 new integration tests across `vm_fiber_sleep`, `vm_fiber_registry`, `vm_fiber_overlap`, `vm_fiber_timeout`).
