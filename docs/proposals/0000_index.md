...
| 0179 | [0179_typeclass_soundness_dictionary_passing_and_associated_types.md](0179_typeclass_soundness_dictionary_passing_and_associated_types.md) | Draft | partial | Stages 0-3 and 5-8 shipped: polymorphic dictionary passing works on both backends, predicates keep their full type arguments end to end, superclass evidence occupies dictionary slots, kinds are checked, associated types reduce or stay stuck, deriving is validated, and `Eq`/`Ord`/`Num`/`Show`/`Semigroup`/`Monoid`/`Functor`/`Applicative`/`Monad` live in `lib/Flow/*.flx`. Stage 4 is done except result-directed *selection*: a call left undetermined between two dictionaries for one class reports `E485` rather than resolving from its expected result, because neither backend has the call's expected type at the selection site. KI-015 and KI-057 to KI-061 closed; KI-070 (lambda annotations) is the only open descendant | complete typeclass direction: one evidence pipeline, sound constraint solving, superclass entailment with dictionary slots, real kind checking, result-directed dispatch (closed KI-015), associated types, safe deriving, and the `Eq→Ord` / `Semigroup→Monoid` / `Functor→Applicative→Monad` hierarchy |
