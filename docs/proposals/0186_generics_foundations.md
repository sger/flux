...
| 6 | done | Land 0185's generalize-by-arity rule on the new foundation. `fn identity(x) { x }` and `fn double(x) { x + x }` are polymorphic; `E490` is preserved by the field-predicate pinning the design called for, which was the 0185 regression. Cost recorded as [KI-089](../known_issues.md#ki-089): an unannotated helper is constrained, so `IAdd` becomes a dictionary call and Aether loses in-place reuse. Landed deliberately — recover it with a specialisation pass, as GHC does, not by refusing to generalize. | suite + parity green |
