...
| 0179 | [0179_typeclass_soundness_dictionary_passing_and_associated_types.md](0179_typeclass_soundness_dictionary_passing_and_associated_types.md) | Draft | partial | monomorphic `__tc_*` dispatch, HKT instances, and contextual instances work; polymorphic dictionary passing crashes at runtime (E1000), solver drops obligations, `src/types/kind.rs` unused | complete typeclass direction: one evidence pipeline, sound constraint solving, superclass entailment with dictionary slots, real kind checking, result-directed dispatch (closes KI-015), associated types, safe deriving, and the `Eq→Ord` / `Semigroup→Monoid` / `Functor→Applicative→Monad` hierarchy |
