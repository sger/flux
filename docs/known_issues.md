...
### KI-050 — Same-class contextual instances dispatch the element to themselves — FIXED
...
Type-directed contextual dictionary resolution now distinguishes container calls from element calls. Container calls use the current mangled instance method and forward the contextual dictionary; element calls extract the method from that dictionary. Verified with the same-class `MyEq<List<a>>` regression through Core/VM and the VM/LLVM parity fixture, plus the `Flow.Json` `Encode<Option<Int>>`, `Encode<List<Int>>`, and `Encode<Array<Int>>` VM regression.
