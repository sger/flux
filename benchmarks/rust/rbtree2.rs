...
enum Tree {
    Leaf,
    Node(Color, Box<Tree>, i128, bool, Box<Tree>),
}
...
fn insert(tree: Tree, key: i128, value: bool) -> Tree {
    if is_red(&tree) {
...
fn mk_map_aux(n: i128, tree: Tree) -> Tree {
    if n == 0 {
        tree
    } else {
        let n1 = n - 1;
...
fn main() {
    let handle = std::thread::Builder::new()
        ...
        .expect("spawn rbtree2 benchmark");
    handle.join().expect("run rbtree2 benchmark");
}
