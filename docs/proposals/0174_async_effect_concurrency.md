...
- Status: Draft (revision 7 — supersedes the original five-phase plan; see "Revision history" at end)
- Proposal PR:
- Flux Issue:
...
| **Phase 1a** — Multi-threaded runtime substrate | ✅ Rust substrate complete | All seven slices landed; outstanding work for the phase is the Flux-source-side `Flow.Task` module + LLVM C-shim wiring (1a-vi follow-up) and ADT auto-derivation for `Sendable` (1a-v follow-up). |
...
| **Phase 1b** — Fiber layer + structured concurrency | 🟡 partial (VM ✅, LLVM 🟡) | **VM path fully functional:** Slices 1b-i through 1b-v landed (effect seams, FiberScheduler, 5 fiber CorePrimOps, Flow.Async source surface). 1b-vi-a/b₁/b₂.1/b₂.2 + `Async.timeout` landed: `Async.sleep` routes through mio reactor; every `Async.run_async` boundary owns a real `FiberScheduler`; `FiberSleep`, `FiberBoth`, `FiberRace`, `FiberTimeout` capture continuations and park/resume through dispatch loop, giving genuine wall-clock overlap. TCP parity (1b-vi-e) completed on VM: both fixtures pass with concurrent server/client. **LLVM path blocked on architecture:** 1b-vi-d (native fiber suspend/resume) requires C ABI shim layer (documented but not implemented). What remains: **C-side shim functions** (1b-vi-d for LLVM), **per-worker queues + multi-OS-thread workers** (1b-vi-c), **cancellation propagation** (race losers / scope cancellation / bracket+timeout on VM). |
...
| 1b-vi-b₂.2 — Concurrent `both` / `race` | ✅ | New `CorePrimOp::FiberBoth = 172` and `FiberRace = 173`. Both spawn two child fibers, park the parent on a synthetic await request id, and the dispatch loop's `on_fiber_done` walks an awaiter index to assemble resume values (tuple for `both`, winner for `race`) before calling `scheduler.complete` on the parent's request. Per-fiber `last_completion_req` lets the dispatch loop look up the right resume value before resuming each parked continuation. Loser cancellation deferred to 1b-vi-c. Acid test: [`vm_fiber_overlap.rs`](../../tests/integration/vm_fiber_overlap.rs) — `both(sleep(500), sleep(500))` ≈ 500ms, `race(sleep(1000), sleep(50))` returns the fast result in ~50ms. |
...
| 1b-vi-c — Multi-worker + cancellation | ⏳ | Per-worker ready queues, OS-thread workers running fibers in parallel, cross-worker completion routing (home-worker enqueue), cancellation propagation (race losers, scope cancellation, bracket cleanup on timeout). Today's VM dispatch loop runs fibers strictly on the calling OS thread; race losers keep running until their backend timer fires (small CPU waste, no observable correctness issue at run_async exit). |
| 1b-vi-d — Native (LLVM) fiber suspend/resume | ⏳ architecture scoped | C ABI shim layer design documented (lines 498-523): `flux_async_suspend`, `flux_async_tcp_connect`, `flux_async_tcp_read`, etc. forward LLVM-generated code to Rust scheduler/AsyncBackend. LLVM emit-side stubs exist (linker fails loudly); **C-side shim functions not yet implemented**. The architecture choice is to **not use pthreads directly** — instead, expose narrow C ABI boundary that routes to the Rust reactor. This preserves one scheduler, one request registry, one completion model across VM and LLVM paths. `flux_fiber_*` C runtime functions (`flux_fiber_run_async`, `flux_fiber_sleep`, `flux_fiber_fork`, `flux_fiber_both`, `flux_fiber_race`, `flux_fiber_timeout`) are scoped as part of the park/resume cycle implementation. |
| 1b-vi-e — Real TCP parity | 🟡 partial (VM only) | **VM path:** [`tests/parity/tcp_listen_accept.flx`](../../tests/parity/tcp_listen_accept.flx) and [`tests/parity/tcp_connect_write_read.flx`](../../tests/parity/tcp_connect_write_read.flx) both pass on VM with concurrent server/client via `both()` and mio reactor. Integration tests [`vm_tcp_echo.rs`](../../tests/integration/vm_tcp_echo.rs) verify round-trip echo and listen/accept/close. **LLVM path:** Blocked on 1b-vi-d (C ABI shim layer). TCP parity fixtures remain marked `// parity: vm` until shim layer ships. |
| **Phase 2** — HTTP/1.1 + JSON + Streams | ⏳ | |
| **Phase 3** — TLS + database client | ⏳ | |
| **Phase 4** — `io_uring` backend (optional) | ⏳ | |

Test count after Phase 1b 1b-vi-a + b₁ + b₂.1 + b₂.2 + timeout + TCP parity: **2493 passed / 0 failed** under `cargo test --all --all-features` (2485 baseline + 8 new integration tests: `vm_fiber_sleep`, `vm_fiber_registry`, `vm_fiber_overlap`, `vm_fiber_timeout`, plus 2 from `vm_tcp_echo`). Parity check: all 65 parity fixtures pass on VM (includes new `tcp_listen_accept` and `tcp_connect_write_read`).
