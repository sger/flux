...
def balance1(left_tree, right_tree):
    ...
    _, left, ky, vy, r2 = right_tree
    if left is not None and left[0] == RED:
        _, l, kx, vx, r1 = left
        return node(RED, node(BLACK, l, kx, vx, r1), ky, vy, node(BLACK, r2, kv, vv, tail))
    if r2 is not None and r2[0] == RED:
        _, l2, kx, vx, r = r2
        return node(RED, node(BLACK, left, ky, vy, l2), kx, vx, node(BLACK, r, kv, vv, tail))
    return node(BLACK, node(RED, left, ky, vy, r2), kv, vv, tail)
...
def balance2(left_tree, right_tree):
    ...
    if left is not None and left[0] == RED:
        _, l, kx1, vx1, r1 = left
        return node(RED, node(BLACK, tree0, kv, vv, l), kx1, vx1, node(BLACK, r1, ky, vy, r2))
    if r2 is not None and r2[0] == RED:
        _, l2, kx2, vx2, r = r2
        return node(RED, node(BLACK, tree0, kv, vv, left), ky, vy, node(BLACK, l2, kx2, vx2, r))
    return node(BLACK, tree0, kv, vv, node(RED, left, ky, vy, r2))
...
def ins(tree, kx: int, vx: bool):
    ...
    if kx < ky:
        if is_red(a):
            return balance1(node(BLACK, leaf(), ky, vy, b), ins(a, kx, vx))
        return node(BLACK, ins(a, kx, vx), ky, vy, b)
    if ky < kx:
        if is_red(b):
            return balance2(node(BLACK, a, ky, vy, leaf()), ins(b, kx, vx))
