...
| 0165 | [0165_io_primop_migration_to_effect_handlers.md](0165_io_primop_migration_to_effect_handlers.md) | Blocked | gap | spike completed 2026-04-23 — blocks on 0170 (polymorphic effect ops) | Route `println`/`print`/`read_file`/etc. through `Perform`; synthesize default handlers around `main`/`test_*` |
| 0170 | [0170_polymorphic_effect_operations.md](0170_polymorphic_effect_operations.md) | Draft | gap | new (2026-04-23), surfaced by 0165 spike | generalize effect-op signatures over free type vars; `effect_op_signatures` stores `Scheme` instead of `TypeExpr`; instantiate fresh at each `perform` site |
