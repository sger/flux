...
| 1b-vi-c — Multi-worker + cancellation | ✅ complete for current scope | VM and native schedulers now cancel race losers, timeout losers, and scoped children at scheduler/backend-request boundaries. Native `scope`/`fork`/`cancel` are Rust scheduler-owned rather than pthread/no-op shims. `FiberScheduler` has per-worker ready queues, round-robin child-fiber assignment, and request-id based completion routing back to the parked fiber's home worker. Native `run_async` owns shared scheduler state, runs non-root logical workers on OS threads, promotes values that cross worker boundaries, and wakes home-worker queues through a condition variable. VM `run_async` now also runs background OS workers: worker VMs share Arc-backed read-only constants/globals, values/fibers crossing workers go through the `ArcValue`/`ArcFiber` mirror, and stealable parked/yielded fibers may migrate while root and immediate FIFO race/first-of ties remain stable. The native generated-code execution lock is gone: C effect/yield state remains TLS, allocation stats are atomic, and worker-thread allocation bypasses the process-global bump arena before touching bump pointers while the root thread keeps the fast path. Cancellation is cooperative for already-running fibers: cancelled fiber ids and backend request ids are suppressed at execution, park, completion, and backend-completion boundaries, so race losers, timeout bodies, and scoped children cannot resume parents after cancellation. |
...
isolated worker VMs. VM fibers inside one `run_async` boundary now run on OS
workers by keeping each worker's stacks/frames local, sharing only Arc-backed
read-only constants/globals, and promoting migrating fibers/values through the
`ArcValue` / `ArcFiber` mirrors. CPU-bound VM fibers can still starve siblings
on their current worker until they suspend or call `Async.yield_now`.

Blockers:

Resolved blockers:

- VM `Value` still uses local `Rc` containers, but cross-worker values are
  promoted to `ArcValue` and demoted on the worker that consumes them.
- Parked fibers promote continuations and effect/evidence state into
  `ArcFiber`; stolen originals are dropped on their owning worker.
- The interpreter frame stack and operand stack stay mutable and local to each
  worker VM; read-only constants/globals are shared through `WorkerSharedState`.
...
- Make resumable VM fiber state `Send`, including continuations, operand stack
  ...
  and `ArcFiber::demote` back; `Fiber::promote_ref` lets the scheduler promote
  a stolen fiber without dropping the original `Rc` graph on the thief thread.
- Split per-fiber execution state from process/global interpreter state so an
  OS worker can resume a fiber without sharing one mutable `Vm`. **Landed:**
  background worker VMs are constructed from `WorkerSharedState` with
  `Arc<[ArcValue]>` constants/globals; constant/global reads demote into local
  `Value`s, while global writes stay local to the worker.
- Route backend completions into real VM OS-worker queues, preserving home
  worker affinity or defining a safe migration policy. **Landed:** background
  workers pop local ready fibers first, then steal the newest parked/yielded
  stealable fiber from another worker by promoting to `ArcFiber`, leaving the
  original on the victim's pending-drop queue. Worker 0 intentionally drains
  only its own queue because it runs on the caller VM rather than a reconstructed
  worker VM.

`FLUX_FIBER_MIGRATION` is now on by default. Set `FLUX_FIBER_MIGRATION=0` to
return to owner-only VM fiber dispatch for diagnostics.
...
- VM dispatch in [`vm/core_dispatch.rs`](../../src/vm/core_dispatch.rs)
  snapshots the task closure across a `Sendable` transfer boundary and runs it
  on a Rust worker thread inside a pooled isolated worker VM. Constants are
  shared through the Arc value mirror and each spawn carries only a sparse
  globals snapshot. The result is rehydrated back into the awaiting or joining
  VM. This gives real parallelism for `Task.spawn` without making ordinary VM
  fiber values thread-safe.
