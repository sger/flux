...
| 0177 | [0177_package_manager.md](0177_package_manager.md) | Partially implemented | partial | Phases 0–1 and Phase 2 resolution shipped 2026-08-24; supersedes 0015 | Package manager: `flux.toml`, `flux.lock`, namespaced packages, registry + store. Phase 0 (`lib/Flume/*`: TOML parser, manifest schema, version arithmetic, resolver), Phase 1 (`flux init/new/build/run/test/check`, path dependencies, namespace-scoped module roots, E469–E471), and Phase 2's resolution half (`Flume.Lock`/`Index`/`Plan`/`Home`: semver resolution against a local registry index, `flux.lock` with checksums, minimal-change updating) are live. Still open in Phase 2: network fetching, `add/update/tree`, `--offline`/`--locked`/`--frozen`, git deps. Phase 3 (store, publishing, workspaces) not started |
