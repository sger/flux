...
use crate::{
    frontend::{block::Block, position::Position},
...
pub fn compile_module_constants(
    ...
    for const_name in &analysis.eval_order {
        ...
        let const_value = eval_const_expr(expr, &local_constants).map_err(|error| {
        ...
        let qualified_name = format!("{}.{}", module_name, const_name);
