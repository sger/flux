...
- ✅ **Parity observation of artifacts**: `src/parity/runner.rs` supports `VmCached` and `LlvmCached` ways that warm caches, re-run, and compare fresh vs cached output. `cargo run -- parity-check ... --ways vm,llvm,vm_cached,vm_strict,llvm_strict` exercises the maintained extended suite.
...
- ✅ **VM == LLVM parity with caching**: Extended parity suite (`vm_cached`, `llvm_cached`, `vm_strict`, `llvm_strict`) now runs on every PR/push in CI (`.github/workflows/ci.yml`) via `cargo run -- parity-check ... --ways ...`.
