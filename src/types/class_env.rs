...
///
/// Almost every instance is [`Declared`](Self::Declared). The exception exists
/// so that one rejected `deriving` clause reports once rather than once per
/// use site — see [`ClassEnv::collect_from_statements`].
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum InstanceOrigin {
    /// Written in source, or generated by an accepted `deriving` clause.
    Declared,
    /// A `deriving` clause already rejected with E486.
    ///
    /// It is recorded so that use sites resolve rather than cascading into a
    /// missing-instance error at each one, but it has no usable methods and
    /// carries no real span, so later validation must skip it.
    RejectedDeriving,
}
...
/// When [`ClassEnv::collect_from_statements_with`] checks that every instance
/// supplies its class's superclass evidence.
///
/// The check needs the complete instance set: an instance's superclass
/// evidence may come from a module this one imports, and those instances are
/// merged after collection.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SuperclassCheck {
    /// Check before returning. Correct when nothing further will be merged.
    Now,
    /// Skip the check; the caller runs
    /// [`ClassEnv::validate_superclass_obligations`] once merging is done.
    Deferred,
}
...
impl ClassEnv {
    ...
    ///
    /// Superclass obligations are checked before this returns. A caller that
    /// merges further instances afterwards — the compiler, which folds in
    /// imported public instances — must use
    /// [`collect_from_statements_with`](Self::collect_from_statements_with)
    /// instead, or evidence living in another module reads as missing.
    ...
    /// over when superclass obligations are checked.
    pub fn collect_from_statements_with(
        ...
        // Proposal 0179 Stage 8: deferred when the caller still has instances
        // to merge. `Ord`'s `Eq` evidence lives in `Flow.Eq`, so checking here
        // would report every stdlib superclass edge as unsatisfied.
        if superclass_check == SuperclassCheck::Now {
...
impl ClassEnv {
    ...
    /// Re-resolve every class's superclass identities now that all classes
    /// are collected.
    ///
    /// `collect_classes` resolves a superclass name against the classes it has
    /// seen so far, so a class declared before its superclass — in an earlier
    /// module block of the same program, say — records
    /// [`ClassId::from_local_name`], which matches no instance and reports the
    /// superclass as unsatisfied. Order of declaration should not decide that.
    fn resolve_superclass_ids(&mut self) {
        let resolved: Vec<(ClassId, Vec<ClassId>)> = self
            .classes
            .values()
            .map(|class| {
                let ids = class
                    ...
                    .iter()
                    .map(|constraint| {
                        self.resolve_class_id(class.module, constraint.class_name)
                            .unwrap_or_else(|| ClassId::from_local_name(constraint.class_name))
                    })
                    .collect();
                (class.class_id(), ids)
            })
            .collect();
        for (id, superclass_class_ids) in resolved {
            if let Some(class) = self.classes.get_mut(&id) {
                class.superclass_class_ids = superclass_class_ids;
            }
        }
    }

    pub fn validate_superclass_obligations(&self, interner: &Interner) -> Vec<Diagnostic> {
        self.validate_superclass_obligations_for(0..self.instances.len(), interner)
    }

    /// [`validate_superclass_obligations`](Self::validate_superclass_obligations)
    /// restricted to a slice of [`Self::instances`].
    ///
    /// A caller that has merged instances imported from other modules must
    /// judge only the ones the current module declares. An imported instance
    /// was validated where it was written, against an environment this one
    /// need not reproduce: `Ord<Int>` comes from `Flow.Ord` carrying its `Eq`
    /// evidence, but a module that imports `Flow.Ord` without `Flow.Eq` would
    /// otherwise report that evidence as missing, at a span in a file it does
    /// not own.
    ///
    /// Class-level cycle detection still spans the whole environment; a cycle
    /// is a property of the class graph, not of one module's instances.
    pub fn validate_superclass_obligations_for(
        &self,
        ...
        for instance in &self.instances[instances] {
            ...
            // A `deriving` clause E486 already rejected. Its superclass
            // evidence is beside the point — and it has no span to report at.
            if instance.origin == InstanceOrigin::RejectedDeriving {
                continue;
            }
            for superclass_id in self.superclass_closure(instance.class_id) {
                ...
                // `instance Eq<Int> => Ord<Int>` names its own evidence. That
                // discharges the obligation here whether or not `Eq<Int>` is
                // visible from this module — the instance is conditional on
                // it, and a use site that cannot supply it reports E444.
                if instance
                    .context_class_ids
                    .iter()
                    .zip(instance.context.iter())
                    .any(|(context_id, constraint)| {
                        *context_id == superclass_id
                            && constraint.type_args.len() == instance.type_args.len()
                            && constraint
                                .type_args
                                .iter()
    ...
    fn try_synthesize_sendable_for_adt(
        ...
        env.instances.push(InstanceDef {
            origin: InstanceOrigin::Declared,
            class_name: sendable_id,
            ...
            match stmt {
                Statement::Data {
                    ...
                    for class_name in deriving {
                        ...
                        let underivable = class_methods.iter().find(|method| {
...
"#;
    ...
    fn env_with_instance(
        ...
        let mut env = ClassEnv::new();
        env.instances.push(InstanceDef {
            origin: super::InstanceOrigin::Declared,
