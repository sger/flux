...
ins :: Tree Integer Bool -> Integer -> Bool -> Tree Integer Bool
...
insert :: Tree Integer Bool -> Integer -> Bool -> Tree Integer Bool
insert t k v =
  if isRed t then setBlack (ins t k v) else ins t k v

mkMapAux :: Integer -> Tree Integer Bool -> Tree Integer Bool
mkMapAux 0 m = m
mkMapAux n m =
  let n' = n - 1
      m' = insert m n' (n' `mod` 10 == 0)
   in seq m' (mkMapAux n' m')

countTrue :: Tree Integer Bool -> Integer
