...
- Implementation notes on expression-level E430: the original plan expected the expression-residue predicate to lean on skolem variables introduced by signature-directed checking (0159). The shipped implementation is simpler — it reuses `fallback_vars` (vars allocated after an inference failure) plus `instantiated_expr_vars` (vars introduced by scheme instantiation at call sites) to gate E430. A var is flagged only when it is (a) not a legitimately generalized `forall` binder in scope, (b) not a call-site instantiation (those resolve via downstream unification), and (c) is a fallback var. Skolems were not required for this gate; they remain part of 0159's checked-mode enforcement path. See [`src/ast/type_infer/static_type_validation.rs::expression_has_unresolved_var`](../../src/ast/type_infer/static_type_validation.rs) for the load-bearing predicate.
