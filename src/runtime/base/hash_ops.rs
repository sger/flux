use crate::runtime::{RuntimeContext, hamt as rc_hamt, hash_key::HashKey, value::Value};
...
fn arg_hamt<'a>(
    ...
    label: &str,
    signature: &str,
) -> Result<&'a std::rc::Rc<rc_hamt::HamtNode>, String> {
    match args[index] {
...
pub(super) fn base_keys_borrowed(
    ...
    let node = arg_hamt(ctx, args, 0, "keys", "argument", "keys(h)")?;
...
pub(super) fn base_values_borrowed(
    ...
    let node = arg_hamt(ctx, args, 0, "values", "argument", "values(h)")?;
    let pairs = rc_hamt::hamt_iter(node);
...
pub(super) fn base_has_key_borrowed(
    ...
    let node = arg_hamt(ctx, args, 0, "has_key", "first argument", "has_key(h, k)")?;
    ...
    let found = rc_hamt::hamt_lookup(node, &key).is_some();
...
pub(super) fn base_merge_borrowed(
    ...
    let node1 = arg_hamt(ctx, args, 0, "merge", "first argument", "merge(h1, h2)")?;
    let node2 = arg_hamt(ctx, args, 1, "merge", "second argument", "merge(h1, h2)")?;

    let pairs = rc_hamt::hamt_iter(node2);
    let mut result = std::rc::Rc::clone(node1);
    for (k, v) in pairs {
...
pub(super) fn base_delete_borrowed(
    ...
    let node = arg_hamt(ctx, args, 0, "delete", "first argument", "delete(h, k)")?;
    let key = args[1].to_hash_key().ok_or_else(|| {
        format!(
...
pub(super) fn base_put_borrowed(
    ...
    Ok(Value::HashMap(rc_hamt::hamt_insert(
        node,
        key,
        args[2].clone(),
    )))
...
pub(super) fn base_get_borrowed(
    ...
    let node = arg_hamt(ctx, args, 0, "get", "first argument", "get(map, key)")?;
    let key = args[1].to_hash_key().ok_or_else(|| {
        format!(
    ...
    match rc_hamt::hamt_lookup(node, &key) {
...
pub(super) fn base_is_map_borrowed(
    ...
    Ok(Value::Boolean(matches!(args[0], Value::HashMap(_))))
}
