...
| 5 | next | `evidence.rs` + `translate.rs`: solver records evidence; delete the other resolution sites. The forwarding reproduction already compiles and runs on this branch; the remaining reachable failure is [KI-090](../known_issues.md#ki-090) — a constrained function referenced *as a value* never has its dictionary applied, because every resolution site is looking at a call. | KI-090's reproduction runs; the six sites are gone |
| 6 | attempted, reverted | Land 0185's generalize-by-arity rule on the new foundation. It works: `fn identity(x) { x }` and `fn double(x) { x + x }` become polymorphic, and the `E490` the 0185 attempt lost is preserved by the field-predicate pinning below — that half is kept. What blocks it is **despecialisation**: an unannotated helper becomes constrained, so `IAdd` lowers to a dictionary-dispatched `Add`, and Aether loses in-place reuse with the concrete type (`my_filter`: `FBIP: fip, FreshAllocs: 0` → `fbip(1), FreshAllocs: 1`). That fails 16 tests which assert superinstruction fusion, `DropSpecialized` elimination for `IntRep` binders, and tail calls still fire, plus 25 snapshots. Landing it now would mean dismantling those guarantees. **A specialisation pass over `core/` comes first**, then the rule re-lands on top of it and those 16 should pass unchanged. | stdlib residue **0**; suite + parity green, *without* weakening an optimisation test |
